# 0006 部署拓扑外置到 config/topology.yaml，去除 pyproject 中的仓库路径

> **决策摘要**：部署拓扑（SSH 目标机、工作目录、要 clone 的 vllm-benchkit/vllm/vllm-ascend 三仓库及 branch/commit）外置到 `config/topology.yaml`；`pyproject.toml` 不再写仓库路径；`deploy.sh` 按清单驱动 clone/checkout/install。
...
| **Supersedes** | — |
| **Related** | ADR-0003（canonical 承载）、ADR-0007（commit 覆盖）、ADR-0005（与运行参数分层） |
| **映射** | 隐私边界（机器/仓库 URL 只在库外 topology.yaml） |
...
- ✅ **已兑现**：`config/topology.example.yaml` 作为模板入库（占位符），`config/topology.yaml` 落地真实值且被 `.gitignore` 忽略；`deploy.sh` 为清单唯一消费方。