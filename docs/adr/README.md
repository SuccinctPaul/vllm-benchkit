...
> 这里是**全部架构决策记录的唯一入口**。每一条 ADR 记录一个"**为什么这么设计**"的决策：背景、候选、取舍、后果，以及它后来有没有兑现。只追加、不改历史；新决策接 `0011` 递增序号。
>
...
guide 黑盒地基（0001–0007，2026-08-20）
  ...
  0005 配置外置 YAML+env ───────────────► 0006（与拓扑分层）
                      │
acceptance 验收（0008–0011，2026-08-24）
...
- **想搞懂运行链路**：按 `0001→0002→0003→0005→0006→0007→0008→0009/0010/0011` 顺着读。
- **只想读决策**：挑 Type 相关的那几条精读，正文按统一的"背景/决策/候选对比/后果/兑现"结构组织。
- **新增决策**：在 [../../docs/adr/](./) 追加 `NNNN-<slug>.md`（序号递增），套用与现有条目一致的前置元数据表；只追加、不改历史。改完回填本文 [决策一览](#决策一览) 一行。