...
> **跑之前先花 30 秒看 [acceptance/how-to-run.md §8 常见坑](./acceptance/how-to-run.md#L106)**——里面有两个不踩必翻车：`--model-ref` 要带组织前缀（`Qwen/...`）才能命中离线缓存；serve 不接受 `--compile-mode`/`--cudagraph-mode`（设 `VLLM_NOTES_SKIP_GRAPH_ARGS=1`）。
