...
这个仓库最初是「成为 vLLM Ascend 插件专家」的**笔记 + 工具仓**：先黑盒跑通官方基准，再逐步拆解。后来在此基础上长出了一个子项目 —— 把《标准交付测试方案》落成一套**正式验收执行系统**（也叫 vllm-xcheck）。
...
```
  ...
  两种玩法：                                           │
   ...
   ② 正式验收（严、留证）◄── acceptance/   按验收方案跑A1–A4，出判定+证据
                        └─────────────────────────────┘
```
...
| **验收（正式判定）** | 按验收方案跑 A1–A4，判通过与证据归档 | [acceptance/README.md](./acceptance/README.md) |

新手别混淆：**冒烟是"能不能跑"，验收是"这套方案我要的东西合不合规"**。前者快、可反复，后者严格、留证据。
